...
Tag, log in, push. Public repository (simplest; no `imagePullSecrets` needed for the cluster to pull it).

## 5. Kubernetes manifests
Root-level `k8s/` (not nested, per the literal deliverable wording — see ADR 0004): `configmap.yaml` (with `DB_SERVER`/`DB_USER` values deliberately different from the local defaults, to prove externalized config works), `deployment.yaml` (liveness/readiness against `/actuator/health`), `service.yaml` (`NodePort`, not `ClusterIP` — see ADR 0002), optional `hpa.yaml`.

## 6. Deploy & validate on Minikube
Dedicated `reto5` namespace. `minikube start --driver=docker`. `kubectl apply -f k8s/`. Validate primarily via `kubectl port-forward` (driver-agnostic and reliable), with `minikube service --url` and `minikube dashboard` as bonus evidence. Confirm `/` returns the ConfigMap values in-cluster, not the jar's local defaults.
